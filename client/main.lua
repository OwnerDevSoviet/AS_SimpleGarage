...
Citizen.CreateThread(function()
	while true do
		...
		for garage, garageData in pairs(Config.Garages) do
			...
			AddTextComponentString("Openbare Garage: " .. garage)
...
function IsVehicleImpounded(plate)
	ESX.TriggerServerCallback('AS_SimpleGarage:checkImpounded', function(isImpounded)	
		if isImpounded == true then
			exports['mythic_notify']:SendAlert('inform', _U('menu_Impounded'), 10000, { ['background-color'] = Config.ColorHex, ['color'] = '#ffffff' })
		end
	end, plate)
end
...
function WaitForModel(model)
	...
	while not HasModelLoaded(model) do
		...
		DrawScreenText("Model aan het laden ", 255, 255, 255, 150)
	end
end
